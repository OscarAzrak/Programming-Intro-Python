...
import string
import re
from memoryClassFileB import Game
from memoryClassFileB import Player
def printIntro():
...
def sizeOfGameAndDifficulty(): #function that asks player what size the player want their board to be, function also checks if the size is okay.
    ...
    while boardSizeOK == False:
        boardSize = input("What width and length do you want your board to be? Width and length has to be same size, answer with only one number \n") #Saves player's input for size in boardSize
...
def checkSizeOK(size, minSize, maxSize): #function that checks wether the size of the board is correct
    ...
    try:
        size = int(size)
        if minSize <= size <= maxSize and size%2 == 0: #answer has to be larger than 2 and be dividable with two, an even number
            okInput = True
        else:
...
def checkIfGameWon(memoryBoard, boardSize, difficulty): #function that checks wether the game has ben completed fully
    gameWon = False
